...
deck_t * hand_from_string(const char * str, future_cards_t * fc) {
  ...
  size_t idx = 0;
  card_t *add_card;
  deck_t *ret = (deck_t *)malloc(sizeof(*ret));
  ret->n_cards = 0;
  ret->cards = NULL;
  ...
  while (str[count] != '\0') {
	...
	add_card = add_empty_card(ret);
	add_future_card(fc, idx, add_card);
	count = lstIdx;
      }
      else
      {
	add_card_to(ret, card_from_letters(str[count], str[count + 1]));
	count += 2;
      }
      g_count++;
    }
  ...
  if (count < 5)
  {
    fprintf(stderr, "Not enough cards (T.T)");
    return NULL;
  }
  return ret;
}
...
deck_t ** read_input(FILE * f, size_t * n_hands, future_cards_t * fc) {
  ...
  {
    ret_arr = (deck_t **)realloc(ret_arr, (*n_hands + 1)*sizeof(*ret_arr));
    lineToStr(line);
    ret_arr[*n_hands] = hand_from_string(line, fc);
    (*n_hands)++;
  }
  free(line);
  return ret_arr;
